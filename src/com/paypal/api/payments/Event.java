...
public class Event extends PayPalResource {
    ...
    /**
     * Name of the event type that occurred on resource, identified by
     * data_resource element, to trigger the Webhooks event.
     */
    private String eventType;

    /**
     * A summary description of the event. E.g. A successful payment
     * authorization was created for $$
     */
    ...
    /**
     * Retrieves the Webhooks event resource identified by event_id. Can be used
     * to retrieve the payload for an event.
     *
...
//    }
    /**
     * Retrieves the list of Webhooks events resources for the application
     * associated with token. The developers can use it to see list of past
     * webhooks events.
     *
    ...
    /**
     * Validates received event received from PayPal to webhook endpoint set for
     * particular webhook Id with PayPal trust source, to verify Data and
     * Certificate integrity. It validates both certificate chain, as well as
     * data integrity.
     *
    ...
    public static boolean validateReceivedEvent(APIContext apiContext, Map<String, String> headers, String requestBody) throws PayPalRESTException, InvalidKeyException, NoSuchAlgorithmException, SignatureException {
        if (headers == null) {
            ...
            logger.info(Constants.PAYPAL_HEADER_TRANSMISSION_SIG + ":" + headers.get(Constants.PAYPAL_HEADER_TRANSMISSION_SIG));
            logger.info(Constants.PAYPAL_HEADER_AUTH_ALGO + ":" + headers.get(Constants.PAYPAL_HEADER_AUTH_ALGO));
            logger.info(Constants.PAYPAL_HEADER_CERT_URL + ":" + headers.get(Constants.PAYPAL_HEADER_CERT_URL));
            logger.info(Constants.PAYPAL_TRUST_CERT_URL + ":" + headers.get(Constants.PAYPAL_TRUST_CERT_URL));
...
//        String trustCertificateLocation = SDKUtil.validateAndGet(cmap, Constants.PAYPAL_TRUST_DEFAULT_CERT);
...
//        logger.info("trustCerts:" + trustCerts);
        ...
        logger.debug("Is Chain Valid: " + isChainValid);
        if (isChainValid) {
            ...
            logger.debug("Is Data Valid: " + isDataValid);
